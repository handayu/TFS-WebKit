...
namespace UseOnlineTradingSystem
{
    ...
    {
        private SelfListed m_commdityInfo;//挂牌
        private Transaction m_delistInfo; //摘牌
        private Transaction m_tradedInfo; //成交
        ...
        public UseMenuTrip(OperationType type, Point screenLocation, ContractCategoryDic vo, SelfListed info, Transaction delistInfo, Transaction tradedInfo, Control OwnerControl)
        {
            InitializeComponent();

            m_commdityInfo = info;
            m_delistInfo = delistInfo;
            m_tradedInfo = tradedInfo;
            m_vo = vo;
        ...
        /// 初始化成交下拉Items
        /// </summary>
        private void InitializeTradedItems()
        {
            Items.Add("生成合同");
            Items.Add("发起订单融资");
            Items.Add("刷新");

            Items[0].Click += USeMenuTripTradedCreateCOntract;
            Items[1].Click += USeMenuTripTradedContractGetCash;
            Items[2].Click += TradedTableRefresh;
        ...
        /// <param name="e"></param>
        private void USeMenuTripTradedCreateCOntract(object sender, EventArgs e)
        {
            string response = HttpService.GetCreateContractInfo(m_tradedInfo.orderNo);
            if (response != null)
            {
                //从注册表中读取默认浏览器的执行文件路径
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void USeMenuTripTradedContractGetCash(object sender, EventArgs e)
        {
            //仅展示用
            OrderTransCash form = new OrderTransCash(m_tradedInfo);
            form.ShowDialog();
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void TradedTableRefresh(object sender, EventArgs e)
        {
            RefrashTradedView();
        }

        private void RefrashTradedView()
        {
