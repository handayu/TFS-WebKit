...
namespace OuterMarketDataStore
{
    public class MQTTMarketDataStoreage : IMarketDataListener, IUSeNotifier
    {
        ...
        private bool m_isSocketRun = false;//监控连接状态
        ...
        private string m_mqName = string.Empty;
        public MQTTMarketDataStoreage(string ipAddressPort, string clientID)
        {
            try
            {
                ...
                //服务器地址后三位
                Debug.Assert(m_mqttServerAddress != "");
                string[] addressArray = m_mqttServerAddress.Split('.');
                m_mqName = addressArray[3].ToString();
                ...
                try
                {
                    ...
                    using (m_mqttClient = new MqttConnection(connArgs))
                    {
                        ...
                        {               
                            ...
                            //[hanyu]暂时只推送上期的品种行情
                            if (marketData.Instrument.Market == USeMarket.SHFE || marketData.Instrument.Market == USeMarket.LME) InternalSendTotMQTT(marketData);
                        }
                    }
                }
                catch (Exception ex)
        ...
        private void InternalSendTotMQTT(USeMarketData marketData)
        {
            try
            {
                ...
                {
                    m_mqttClient.Publish(m_topicLME, body, StriderMqtt.MqttQos.AtMostOnce, false);
                }
                else
                {
                    m_mqttClient.Publish(m_topic, body, StriderMqtt.MqttQos.AtMostOnce, false);
                }
            ...
            catch (Exception ex)
            {
                ...
                string text = string.Format("** {0}发送MQTT实时行情数据失败,{1}", this.StoreageName, ex.Message);
                m_eventLogger.WriteError(text);
                USeNotifyEventArgs notify = new USeNotifyEventArgs(USeNotifyLevel.Warning, text);
                SafeRaiseNotifyEvent(this, notify);
            }
