...
{
    /// <summary>
    /// 期货合约信息下载处理类,按照时间序列Insert到contract_history-不覆盖。
    /// </summary>
    public class DownloadServiesHistory
    {
        #region member
        ...
        {
            string cmdText = string.Format(@"insert into {0}.contracts_history(date_time,contract,contract_name,exchange,open_date,expire_date,
...
VALUES(@date_time,@contract,@contract_name,@exchange,@open_date,@expire_date,
@start_deliv_date,@end_deliv_date,@volume_multiple,@is_trading,@varieties,@price_tick,
@exchange_long_margin_ratio,@exchange_short_margin_ratio,@product_class,@underlying_instrument,@max_market_order_volume,
@min_market_order_volume,@max_limit_order_volume,@min_limit_order_volume);",
                m_alphaDBName);
